...
function M:on_deposit_asset_imp(jsonstrArgs: string)
	...
	if op == "openCdc" then
		...
		if wishGetStableTokenAmount >= maxStableTokenAmount then
			return error("wishGetStableTokenAmount(".. tostring(wishGetStableTokenAmount) .. ") must < " ..tostring(maxStableTokenAmount))
		end
...
function M:payBack(arg:string)
	...
	cdc_info['secSinceEpoch'] = get_chain_now()
	var repay_principal:int = payBackAmount - fee  
